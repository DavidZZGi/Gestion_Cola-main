...
for (var item in result) {
  Shop shop =Shop(name: item['nombre'], id: item['id'], activa: item['activa'], idMunicipio: item['id_municipio']);
  shops.add(shop);
}
return shops;
...
List<Shop>shops=[];
Response response =await http.get(Uri.parse('http://localhost:3001/tiendas/todas/2301'));
if(response.statusCode==200){
  final result=jsonDecode(response.body);
  for (var item in result) {
    Shop shop =Shop(name: item['nombre'],
     id: item['id'], 
     activa: item['activa'], 
